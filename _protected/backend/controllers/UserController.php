...
class UserController extends BackendController
{
    ...
    public function actionCreate()
    {
            ...
            Model::validateMultiple([$user, $role]))
        {
            ...
            $user->picture = $user->uploadProfile($user, 'picture');
            if ($user->save()) 
            {
    ...
    public function actionUpdate($id)
    {
            ...
            $role->load(Yii::$app->request->post()) && Model::validateMultiple([$user, $role])) 
        {
            ...
            $image = UploadedFile::getInstance($user, 'picture');


            if (!empty($image) && empty($t) ) {
                $user->picture = $user->uploadProfile($user, 'picture');
                echo 'if';
            } elseif(!empty($image) && !empty($t)) {
                $remove = $user->pictureRemove($t);
                $user->picture = $user->uploadProfile($user, 'picture');
